...
let prefix = await db.fetch(`prefix.${message.guild.id}`) || fynx.prefix  
  ...
  return client.channels.get(lh).send(`:scroll: <@${message.author.id}> İçin Kayıt İşlemi Başarı İle Tamamlandı. `)
  }
  let isim = args[0]
  let yas = args[1];
  ...
  client.channels.get(lh).send(`:scroll: <@${message.author.id}> İçin Kayıt İşlemi Başarı İle Tamamlandı. `)
}
exports.config = {
 name: "kayıt-ol",
